...
void empleadoMasGanaPorSector(eEmpleado empleado[],int tam, eSector sector[],int tamSector);
int menu();
...
int main()
{
    ...
    do
    {
        ...
        case 5:
            ...
            break;
        case 7:
            empleadoMasGanaPorSector(lista,10,sectores,5);
            break;
        case 10:
            seguir = 'n';
            break;
...
int menu()
{
    ...
    printf("7- Listar empleados que mas ganan por sector\n ");
    printf("10- Salir\n");
...
void empleadoMasGanaPorSector(eEmpleado empleado[],int tam, eSector sector[],int tamSector)
{
    int i,j, indexEmpleado;
    float max;

    /*for(j=0; j< tam; j++){
        printf("\n %d %s %c %.2f \n",empleado[j].legajo,empleado[j].nombre,empleado[j].sexo,empleado[j].sueldo);
    }*/
    ...
    for(i=0; i<tamSector; i++)
    {
        ...
        for(j=0; j<tam; j++)
        {
            if(sector[i].id == empleado[j].idSector && empleado[j].isEmpty != 0)
            {
                if(j==0 || empleado[j].sueldo > max)
                {
                    max = empleado[j].sueldo;
                    indexEmpleado = j;
                    /* cuando termina debo guarda la iteracion donde esta ubicado el indice del sueldo mayor
                        asi puedo imprimir los valores asociados a este, si no guardo el indice me imprimira basura.
                    */
                }
            }
        }
        printf("\n %s \n %d %s %c %.2f \n",sector[i].descripcion,empleado[indexEmpleado].legajo,empleado[indexEmpleado].nombre,empleado[indexEmpleado].sexo, max);
    }
