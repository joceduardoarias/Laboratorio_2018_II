...
LinkedList* ll_map(LinkedList* this, int (*pFunc)(void*))
{
    ...
    if(this != NULL && pFunc != NULL && !ll_isEmpty(this))
    {
        ...
        for(i=0; i<ll_len(this); i++)
        {
            if(pFunc(ll_get(this,i)) < 177)
            {
                pElement = ll_get(this,i);
                sueldo = pElement->horasTrabajadas * 180;
                employee_setSueldo(pElement,sueldo);
                ll_add(this2,pElement);
            }
            else
            {
                if(pFunc(ll_get(this,i)) > 176 && pFunc(ll_get(this,i)) < 209 )
                {
                    pElement = ll_get(this,i);
                    sueldo = pElement->horasTrabajadas * 270;
                    employee_setSueldo(pElement,sueldo);
                    ll_add(this2,pElement);
                }
                else
                {
                    if(pFunc(ll_get(this,i)) > 208 && pFunc(ll_get(this,i)) < 241 )
                    {
                        pElement = ll_get(this,i);
                        sueldo = pElement->horasTrabajadas * 360;
                        employee_setSueldo(pElement,sueldo);
                        ll_add(this2,pElement);
                    }
                }
